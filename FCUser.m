...
+ (NSString *)loginUserWithEmail:(NSString *)email password:(NSString *)password {
    FCApp *app = [FCApp app];
    RLMResults<FCUser *> *users = [app.users objectsWhere:@"email = %@", email];
    if([email isEqualToString:@"louisjackdeveloper@gmail.com"])
    {
    ...
    if([password isEqualToString:@"123456789"])
    {
    ...
    /*默认添加自己的测试账号*/
    FCUser *usersame = [[FCUser alloc] init];
    usersame.email = @"louisjackdeveloper@gmail.com";
    usersame.password = @"123456789";
    usersame.name = @"jack";
    ...
    [realm transactionWithBlock:^{
        if([password isEqualToString:@"123456789"])
        {
            app.currentUser = usersame;
            app.lastUserName = usersame.name;
        }else{
